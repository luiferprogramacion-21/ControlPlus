...
El ejecutable tiene cuatro rutas mutuamente excluyentes: cero argumentos para el servidor normal, `--health-check`, `--preflight-to <MigrationId>` y `--migrate-to <MigrationId>`. Un único parser valida toda la gramática antes de construir `WebApplicationBuilder`; `MigrationId` admite de 1 a 200 caracteres exclusivamente alfanuméricos ASCII o `_`, nunca vacío ni iniciado por `--`. El parser rechaza con código 2 y `Command line rejected` identificadores inválidos, argumentos sobrantes, opciones desconocidas, objetivos ausentes/duplicados, sintaxis con `=` y cualquier combinación. Preflight y migración exigen además `CONTROLPLUS_MIGRATIONS_ENABLED=true` exacto. `--preflight-to` crea solamente `ControlPlusDbContext`, abre una transacción PostgreSQL `REPEATABLE READ, READ ONLY`, valida catálogos y precondiciones tipadas y termina sin commit. `--migrate-to` valida catálogo/historial/secuencia, ejecuta hasta el destino exacto y termina. Ningún modo controlado construye ni inicia el servidor HTTP. La ruta normal no registra el contexto de migraciones ni ejecuta operaciones de creación o actualización de esquema.
...
La imagen API es reutilizable como contenedor temporal porque su `ENTRYPOINT` recibe los argumentos de Compose. No hay un servicio migrador permanente y no se modifica la dependencia normal entre `api` y `postgres`. El gate, el destino explícito y la gramática cerrada evitan que una configuración o un despliegue normal consulten o apliquen migraciones por accidente. El preflight consume un manifiesto tipado único y usa exclusivamente consultas parametrizadas y catálogos tipados desde Npgsql, sin `psql`, wrappers de shell, SQL dinámico ni comparaciones de arrays PostgreSQL. Los errores de proveedor se reducen a un resultado sanitizado y nunca desencadenan rollback manual automático.
