...
9. Nunca aplicar migraciones al iniciar la API. El único modo operativo aprobado exige simultáneamente `CONTROLPLUS_MIGRATIONS_ENABLED=true` y `--migrate-to <MigrationId>` desde un contenedor temporal, con respaldo y autorización previos para cualquier base persistente. Producción requiere autorización explícita.
...
- La imagen API incorpora un migrador controlado y reutilizable. `docker compose run --rm --no-deps -e CONTROLPLUS_MIGRATIONS_ENABLED=true api --migrate-to <MigrationId>` usa la configuración normal de Compose, valida que el historial sea un prefijo coherente y solo aplica una secuencia ascendente hasta el destino exacto. El modo no inicia HTTP, no acepta “todo lo pendiente”, no revela secretos y no intenta rollback manual. No está autorizado aplicarlo ahora sobre PostgreSQL persistente.
